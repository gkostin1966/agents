...
> Q32, open `AGENT_QUIZ_ANSWERS.md` and grade the answers yourself, or ask the agent
...
**Q31.** Six domain events in the `preservation` module are annotated `@Externalized`
and published to RabbitMQ. List all six event class names and, for each, the exchange
it routes to (use the exchange name string, not the Java constant name).
Why are the `IngestValidation*` outcome events **not** externalized?

*(Hint: grep for `@Externalized` in `src/main/java/`; `AmqpIntegrationConfiguration.java`)*

---

**Q32.** The application requires a Spring profile to enable authentication.
Name both available profiles, the security mechanism each one activates, and the
exact command to start the application locally using the `dev` profile.

*(Hint: `AGENTS.md` § Java / Gradle Conventions — Spring profiles;
`src/main/java/edu/umich/lib/dor/depot/BasicSecurityConfig.java`;
`src/main/java/edu/umich/lib/dor/depot/WebloginSecurityConfig.java`)*

---

## When You Have Answered All 32 Questions

Stop here. Do **not** open `AGENT_QUIZ_ANSWERS.md`.

Tell the developer:

> "I have answered all 32 quiz questions. Please open `AGENT_QUIZ_ANSWERS.md` to grade
