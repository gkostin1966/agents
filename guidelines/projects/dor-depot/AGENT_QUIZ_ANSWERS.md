...
**`PerformIntegrityCheck`** (listener: `on(PerformIntegrityCheck)`):
Calls `preservationGateway.checkIntegrity()` (OCFL object validation) and stores a PREMIS
`FIXITY_CHECK` event, then publishes one of:
- **`IntegrityCheckPassed`** — object validated successfully (no errors).
- **`IntegrityViolationDetected`** — validation returned errors (fixity failure).
...
Note: `IntegrityCheckPassed`, `IntegrityViolationDetected`, and `IntegrityCheckTargetMissing` are
annotated `@Externalized` and published to RabbitMQ. The `IngestValidation*` events are **not**
externalized — they remain internal Spring application events only.
...
**A31.** Six events in the `preservation` module carry `@Externalized` and are
published to RabbitMQ:

| Event class                   | Exchange               |
|-------------------------------|------------------------|
| `BinContentsModified`         | `intake.exchange`      |
| `SubmissionRejected`          | `intake.exchange`      |
| `IntegrityCheckPassed`        | `integrity.exchange`   |
| `IntegrityViolationDetected`  | `integrity.exchange`   |
| `IntegrityCheckTargetMissing` | `integrity.exchange`   |
| `PublicationFailed`           | `publication.exchange` |

The `IngestValidation*` events (`IngestValidationPassed`, `IngestValidationDiscrepancyDetected`,
`IngestValidationTargetMissing`) are **not** externalized — they carry no `@Externalized`
annotation and remain internal Spring application events. No downstream system currently
consumes ingest-validation outcomes via RabbitMQ.

*(Source: grep `@Externalized` in `src/main/java/`; `AmqpIntegrationConfiguration.java`)*

---

**A32.** Two Spring profiles enable authentication:

- **`dev`** — HTTP Basic / form-based login via `BasicSecurityConfig`; credentials
  are set in `application-dev.properties` (`spring.security.user.name`, `.password`, `.roles`).
- **`demo`** — OIDC via U-M Weblogin via `WebloginSecurityConfig`; configured via
  `application-demo.properties` (`weblogin.issuer.uri`, `.client.id`, `.client.secret`).
...
*(Source: `AGENTS.md` § Java / Gradle Conventions — Spring profiles;
`BasicSecurityConfig.java`; `WebloginSecurityConfig.java`)*
