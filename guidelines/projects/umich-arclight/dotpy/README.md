...
`.agents/dotpy/commit_msg.txt` is never committed, so
...
The script reads `.agents/dotpy/commit_msg.txt`, writes its contents to a temp file via
...
Generates `.agents/emails/its-oidc-request.rtf` — a macOS TextEdit-compatible RTF draft
...
Takes no arguments. Output path is hardcoded to `.agents/emails/its-oidc-request.rtf`.
