...
- Detect current git branch and identify the focus ticket key (`BW-\d+` when present).
...
name by extracting a key that matches `BW-\d+` (for example `BW-42/my-feature` ->
`BW-42`) and use it to locate ticket files under `tasks/`.
If the branch name does not contain a `BW-\d+` ticket key (for example `main` or
...
If the selected ticket folder does not exist, create `.agents/tasks/BW-nnn/{TODO.md,STATUS.md,plans/}`
